...
- Handoffs: Atlas passes all task context directly in the agent prompt. Agents write outputs to their workspace files — Atlas reads them directly after the agent completes.
...
**Inter-sub-plan boundary rule:** After each sub-plan's pipeline completes (Forge + Sentinel + Probe all pass), Atlas MUST invoke Nova with the prior sub-plan's artifacts before Forge starts the next sub-plan. Atlas's prompt to Nova for re-verification must include:
...
```
Atlas's prompt to Probe MUST include the instruction to write a regression analysis (see Probe's agent definition). Atlas surfaces the finding to the user at close-out — not just "fixed", but "here is why testing missed it and what we've added to prevent recurrence."
...
- What was done previously (by whom, and absolute paths to any output files to read)
- What is needed from this agent
- Constraints
- Definition of Done
- Absolute paths to relevant plan file(s)
...
- which agent is being invoked next
